...
func TestVisibility_Do(t *testing.T) {
	...
	p := NewVisibility(xlogger.NewNoop(), cm, res.StreamRegistry, res.ConsumerRegistry, in)
...
func TestVisibility_Do_NoVisiableTask(t *testing.T) {
	ctx := context.Background()
	conn, err := tester.SetupPostgres(ctx)
	defer func() {
		require.NoError(t, conn.Close(ctx))
...
func TestVisibility_Do_NoTask(t *testing.T) {
	ctx := context.Background()
	conn, err := tester.SetupPostgres(ctx)
	defer func() {
		require.NoError(t, conn.Close(ctx))
